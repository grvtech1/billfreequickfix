...
export default async function handler(req, res) {
  ...
  if (await rateLimited(req, { max: 120, windowSec: 60 })) {
  ...
  try {
    ...
    const safeQ = redactPII((q || '').slice(0, 120)); // strip phone/MID before it hits logs or KV
    console.log(JSON.stringify({ kind: 'event', t: Date.now(), event, q: safeQ, results, id }));

    if (kvReady) {
      if (event === 'search' && q) {
        const term = safeQ.toLowerCase().slice(0, 80);
